...
Parses the source .md, extracts the numbered bibliography (or an author-date reference
list), collects every in-text citation from the prose, and cross-checks the two. This
mechanizes the scrambled-bibliography detection the method used to do entirely by hand:
numbered references frequently do NOT match the surnames used in the prose, and citing
by the wrong entry is the exact failure the citations phase exists to prevent.

Output JSON (write with --out, else stdout):
  {
    "entries":  {"1": {"raw": "...", "surnames": ["Park", "Choo"], "year": "2024"}, ...},
    "in_text":  {"numbered": ["1", "3", ...], "author_date": ["(Park & Choo, 2024)", ...]},
...
def main():
    ...
    warnings = []
    if not entries:
        warnings.append("no numbered bibliography found in source")
    ...
    if args.out:
        ...
        print(f"Wrote {args.out}: {len(entries)} entries, "
              f"{len(numbered)} numbered + {len(author_date)} author-date in-text cites, "
